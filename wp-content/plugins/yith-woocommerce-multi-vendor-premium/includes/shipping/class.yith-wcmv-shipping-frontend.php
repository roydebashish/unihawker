    ...
    class YITH_Vendor_Shipping_Frontend  {
			    ...
			    foreach( $matched_zone_keys as $zone_key ){
				    if( ! empty( $vendor->zone_data[ $zone_key ]['zone_post_code'] ) ){
					    ...
					    foreach( $postcodes as $postcode ){
						    $postcode = sanitize_title( $postcode );
        ...
        private function get_extra_cost( $vendor , $package, $shipping_method ) {
            ...
            foreach ( $products as $product ) {
	            $check = false;
                $shipping_product_qty_price_total_amount  += ( ( $product['quantity'] - 1 ) * $vendor->shipping_product_qty_price );
	        ...
	        $shipping_class_type = isset( $shipping_method['type'] ) ? $shipping_method['type'] : 'class';
        ...
        private function is_enable_rate( $package , $is_free_shipping , $shipping_method , $is_counpon_free_shipping ) {
            ...
            if( $is_free_shipping ) {
	            ...
	            if ( in_array( $shipping_method['method_requires'], array( 'min_amount', 'either', 'both' ) ) && $total >= $min_amount ) {
