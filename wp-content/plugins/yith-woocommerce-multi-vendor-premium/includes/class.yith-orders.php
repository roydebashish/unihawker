	...
	class YITH_Orders {
		...
		public function create_suborder( $parent_order, $vendor_id, $vendor_products, $posted ) {
			...
			if( $is_wpml_configured ){
				...
				update_post_meta( $suborder_id, 'wpml_language', $wpml_language_from_parent_order );
			}
		...
		public static function get_suborder( $parent_order_id = false, $check_created_via = false ) {
			...
			if( $check_created_via ){
				$temp_suborder_ids = $suborder_ids;
				foreach( $suborder_ids as $k => $suborder_id ){
					$created_via = get_post_meta( $suborder_id, '_created_via', true );
		...
		public function woocommerce_email_enabled_new_order( $enabled, $object ) {
			...
			return $enabled && $object instanceof WC_Order && wp_get_post_parent_id( yit_get_prop( $object, 'id' ) ) != 0 ? false : $enabled;
		}
