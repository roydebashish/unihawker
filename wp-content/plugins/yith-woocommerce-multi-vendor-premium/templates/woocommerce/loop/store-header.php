...
if( ! empty( $vendor->header_image) && 'small-box' == $header_skin && 'background' == $hedaer_image_html && ! empty( $header_image[0] ) ){
    $wrapper_style = "background: url({$header_image[0]}) top center; background-size: cover;";
}
?>

<div class="<?php echo $store_header_class . ' ' . $header_skin  ?>" style="<?php echo $wrapper_style ?>">
    <!--  Header Image -->
    <?php if( ! empty( $vendor->header_image) && ( 'image' == $hedaer_image_html || 'double-box' == $header_skin ) ): ?>
    ...
    <div class="store-socials">
        <span class="socials-container">
            <?php foreach( $vendor->socials as $social => $uri ) : ?>
                <?php if( 'website' == $social && 'header' == $website['show'] ) { continue; }?>
                <?php if( ! empty( $uri ) ) : ?>
