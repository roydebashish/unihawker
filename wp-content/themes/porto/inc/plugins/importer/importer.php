...
add_action( 'wp_ajax_porto_import_dummy_step_by_step', 'porto_import_dummy_step_by_step' );
add_action( 'wp_ajax_porto_import_revsliders', 'porto_import_revsliders' );
...
add_filter( 'wp_import_existing_post', 'porto_import_override_contents', 10, 2 );
add_action( 'import_start', 'porto_import_dummy_start' );
add_action( 'import_end', 'porto_import_dummy_end' );
...
function porto_import_dummy() {
    ...
    if ( current_user_can( 'manage_options' ) && class_exists( 'WP_Importer' ) && class_exists( 'WP_Import' ) ) { // check for main import class and wp import class
        ...
        porto_import_before_functions( $demo );
        ...
        if ( 'import_start' == $process && $response ) {
            ...
            porto_import_after_functions( $demo );
        }
    }
    die();
}

function porto_import_override_contents( $post_exists, $post ) {
...
function porto_import_dummy_start() {
    if ( current_user_can( 'manage_options' ) ) {
        delete_option( 'porto_import_processed_duplicates' );
    }
}

function porto_import_dummy_end() {
...
function porto_import_dummy_step_by_step() {
    if ( !defined('WP_LOAD_IMPORTERS') ) define('WP_LOAD_IMPORTERS', true); // we are loading importers
...
function porto_import_revsliders() {
    if ( current_user_can( 'manage_options' ) ) {
        $demo = (isset($_POST['demo']) && $_POST['demo']) ? $_POST['demo'] : 'landing';
...
function porto_import_before_functions( $demo ) {
    if ( $demo != 'shortcodes') {
...
function porto_import_after_functions( $demo ) {
    delete_option( 'porto_import_processed_duplicates' );
    if ( $demo != 'shortcodes' ) {
